    ...
    >
      ...
      <div className="bg-white shadow-xl rounded-3xl p-6 mb-6 flex flex-col md:flex-row items-center gap-6">
        <img
          ...
          alt="Profile"
          className="w-36 h-36 rounded-full object-cover border-4 border-blue-600 shadow"
        />
        <div className="text-center md:text-left space-y-2">
          <h2 className="text-3xl font-bold text-gray-800">{user?.displayName || 'User Name'}</h2>

          {user?.role && user.role !== 'user' && (
            <span className="inline-flex items-center gap-2 text-sm bg-blue-100 text-blue-700 px-4 py-1 rounded-full font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cards.map((card, index) => (
          <motion.div
            key={index}
            className="bg-white shadow-md rounded-xl p-4 flex items-start gap-4 border-l-4 border-blue-500"
